...
#  other. Scenario 2 requires bubbles around dots that are social          #
#  distancing and no bubbles around dots that are ignoring the order       #
#  90% of bubbles will follow the order and 10$ will not. If two bubbles   #
#  that are following SD6 collide and remain 6 ft apart, they do not get   #
#  sick.                                                                   #
#                                                                          #
...
for b in range(94):  # creates 94 blues dots
    ...
    if 1 <= chance_sd6 <= 10:  # dot does not follow sd6 and has no bubble (10%)
        blues.add(dot.Dot("images/blueDot.png", 30, pt1, pt2, 0))
    else:
...
for g in range(5):  # creates 5 green dots
    ...
    chance_sd6 = (random.randint(1, 100))  # random chance of person following sd6
    if 1 <= chance_sd6 <= 10:  # dot does not follow sd6 and has no bubble (10%)
        greens.add(dot.Dot("images/greenDot.png", 30, pt1, pt2, 0))
    else:
        greens.add(dot.Dot("images/greenBubble.png", 30, pt1, pt2))

# create the one red dot that does not follow SD6
pt_r1 = ((random.randint(0, 500)), (random.randint(0, 500)))
pt_r2 = ((random.randint(0, 500)), (random.randint(0, 500)))
reds.add(dot.Dot("images/redDot.png", 30, pt_r1, pt_r2, 0))
